...
export class FroggerScene implements Scene {
  ...
  private createLanes(): Lane[] {
    ...
    lanes.push({ y: 19, type: 'safe', obstacles: [], spawnRate: 0, direction: 1, speed: 0 });
    lanes.push({ y: 18, type: 'safe', obstacles: [], spawnRate: 0, direction: 1, speed: 0 });

    // Road lanes (rows 13-17)
    lanes.push({ y: 17, type: 'road', obstacles: [], spawnRate: 20, direction: 1, speed: 0.5 });
    lanes.push({ y: 16, type: 'road', obstacles: [], spawnRate: 25, direction: -1, speed: 0.3 });
    lanes.push({ y: 15, type: 'road', obstacles: [], spawnRate: 18, direction: 1, speed: 0.4 });
    lanes.push({ y: 14, type: 'road', obstacles: [], spawnRate: 22, direction: -1, speed: 0.6 });
    lanes.push({ y: 13, type: 'road', obstacles: [], spawnRate: 30, direction: 1, speed: 0.35 });

    // Safe middle (row 12)
    lanes.push({ y: 12, type: 'safe', obstacles: [], spawnRate: 0, direction: 1, speed: 0 });

    // Water lanes (rows 7-11)
    lanes.push({ y: 11, type: 'water', obstacles: [], spawnRate: 25, direction: -1, speed: 0.3 });
    lanes.push({ y: 10, type: 'water', obstacles: [], spawnRate: 20, direction: 1, speed: 0.4 });
    lanes.push({ y: 9, type: 'water', obstacles: [], spawnRate: 30, direction: -1, speed: 0.25 });
    lanes.push({ y: 8, type: 'water', obstacles: [], spawnRate: 22, direction: 1, speed: 0.35 });
    lanes.push({ y: 7, type: 'water', obstacles: [], spawnRate: 28, direction: -1, speed: 0.3 });

    // Goal zone (rows 0-6)
    for (let y = 0; y <= 6; y++) {
      lanes.push({ y, type: 'goal', obstacles: [], spawnRate: 0, direction: 1, speed: 0 });
    }
